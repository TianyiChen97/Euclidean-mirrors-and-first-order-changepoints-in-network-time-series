...
###### The table 1 comparison results is generated using codes below
...
out_dd <- foreach(mc = 1:nmc) %do% {
  ...
  df <- df %>%
    ...
    mutate(
      mod_leiden = map_dbl(g, ~ {
        comm <- cluster_leiden(.x, objective_function = "modularity")
        modularity(.x, membership(comm))
      }),
      mod_louvain = map_dbl(g, ~ {
        comm <- cluster_louvain(.x)
        modularity(.x, membership(comm))
      })
    )
  ...
  tmp_avg_edges <- linf_error(sqrt_edges)[1]
  
  tmp_mod_leiden <- linf_error(df$mod_leiden)[1]
  tmp_mod_louvain <- linf_error(df$mod_louvain)[1]
  ...
  for (k in 1:3) {
    ...
    tmp1[k] <- linf_error(df.iso)[1]
  }
  
  example_df <- if (mc %% 100 == 0) df else NULL
  
  list(tmp1 = tmp1, 
       tmp_avg_edges = tmp_avg_edges,
       tmp_mod_louvain = tmp_mod_louvain,
       tmp_mod_leiden = tmp_mod_leiden,
       example_df = example_df)
}

print(difftime(Sys.time(), a, units = "hours"))

err_dMV <- Reduce('cbind', lapply(out_dd, "[[", "tmp1"))

err_leiden <- matrix(sapply(out_dd, "[[", "tmp_mod_leiden"), nrow = 1)
err_louvain <- matrix(sapply(out_dd, "[[", "tmp_mod_louvain"), nrow = 1)

err_avg_edges <- matrix(sapply(out_dd, "[[", "tmp_avg_edges"), nrow = 1)

all_err <- rbind(err_dMV, err_leiden, err_louvain, err_avg_edges)

all_err_sq <- abs(all_err)^2

err_mean <- apply(all_err_sq, 1, mean)
err_sd   <- apply(all_err_sq, 1, sd)

err_lower <- err_mean - 1.96 * err_sd / sqrt(nmc)
err_upper <- err_mean + 1.96 * err_sd / sqrt(nmc)

summary_table <- data.frame(
  Method = c("dMV with d = 1", 
             "dMV with d = 2", 
             "dMV with d = 3", 
             "Leiden Modularity", 
             "Louvain Modularity",
